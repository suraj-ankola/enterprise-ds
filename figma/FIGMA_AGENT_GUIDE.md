...
> **Storybook source:** `src/components/ui/` and `src/components/marketing/`  
...
### Phase 1 — Atoms
1. **Icon** — 16×16 frame, Phosphor icons (Regular + Fill weights)
2. **Badge** — variants: neutral/brand/success/warning/danger/info, sizes: sm/md
...
6. **Input** — height: 36px, sizes: sm/md/lg, states: default/focus/error/success/disabled
7. **Select** — same sizing as Input, with caret icon
8. **Textarea** — min-height: 80px, resize handle
9. **Checkbox** — 14×14, states: unchecked/checked/indeterminate/disabled
10. **Radio** — 14×14, states: unselected/selected/disabled
11. **Toggle** — 36×20 track, 16×16 thumb, states: off/on/disabled, sizes: sm/md
12. **Slider** — track 4px, thumb 16×16
13. **Quantifier** — number input + +/- buttons

### Phase 3 — Buttons
14. **Button** — variants: primary/secondary/ghost/danger/link, sizes: sm/md/lg, states: default/hover/focus/disabled/loading
15. **IconButton** — square, same variants + sizes as Button

### Phase 4 — Feedback
16. **Alert** — variants: info/success/warning/danger, with/without icon, dismissible
17. **Toast/UndoToast** — same variants, with undo action slot
18. **Progress** — linear bar, variants: brand/success/warning/danger, sizes: xs/sm/md
19. **Skeleton** — variants: text/rect/circle, configurable width/height

### Phase 5 — Overlay
20. **Tooltip** — 4 positions, max-width: 240px, delay indicator
21. **Popover** — 8 positions, arrow pointer, shadow/xl
22. **Modal** — sizes: sm(400)/md(560)/lg(720)/xl(900)/full, header/body/footer slots
23. **Drawer** — sides: left/right/top/bottom, sizes: sm/md/lg/xl

### Phase 6 — Navigation
24. **Tabs** — variants: line/pill/boxed, sizes: sm/md, full-width option
25. **Breadcrumb** — with/without icons, separator: slash/chevron
26. **Pagination** — prev/next + page numbers, sizes: sm/md
27. **Sidebar** — collapsed(48px)/expanded(224px), with groups + active state
28. **WorkspaceSwitcher** — collapsed/expanded modes, dropdown

### Phase 7 — Data Display
29. **Table** — header row, data rows, sortable column, hover state
30. **DataGrid** — with column resize handles, row selection checkbox
31. **StatCard** — value + label + trend delta, loading skeleton
32. **Card** — variants: default/bordered/elevated/ghost, padding options
33. **EmptyState** — sizes: sm/md/lg, with/without icon, with/without action

### Phase 8 — Complex
34. **FilterBar** — search + filter chips row
35. **CommandPalette** — full-screen overlay, search input + grouped results
36. **Accordion** — variants: default/flush/card, single/multi expand
37. **Timeline** — vertical/horizontal layouts, status dots
...
Row 4: SettingsPage | UserManagementPage
Row 5: AuditLogPage | BillingPage
Row 6: NotificationsCenter | ReportsPage
Row 7: OnboardingFlow (each step) →
Row 8: Marketing — HeroSection variants
Row 9: Marketing — Footer + MarketingNav
Row 10: Marketing — ErrorPage variants (404, 500, 503, maintenance)
```
